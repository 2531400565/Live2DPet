...
public class CubismEyeBlink
{
    ...
    {
        // 注意：原版 C++ 为 `rand() / RAND_MAX`（浮点除法，结果 0..1）。
        // C# 移植时误写成 `_random.Next() / int.MaxValue`（整数除法，恒为 0），
        // 导致 `_nextBlinkingTime == _userTimeSeconds`，睁眼结束后的下一帧立即再次闭眼，
        // 表现为"眨得飞快/不停眨眼"。这里改用浮点除法，恢复 0..1 的随机间隔。
