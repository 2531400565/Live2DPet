...
#pragma warning restore SYSLIB1054 // 使用 “LibraryImportAttribute” 而不是 “DllImportAttribute” 在编译时生成 P/Invoke 封送代码
    ...
    /// <param name="model">Model to query.</param>
    /// <returns>Valid pointer on success; '0'otherwise.</returns>
    // NOTE: Cubism Core 5/6 renamed csmGetDrawableRenderOrders -> csmGetRenderOrders.
    // The vendored engine was ported from SDK 4.2 which still uses the old name.
    // Same semantics/signature (model -> csmInt32* render-order array), so we rebind here.
