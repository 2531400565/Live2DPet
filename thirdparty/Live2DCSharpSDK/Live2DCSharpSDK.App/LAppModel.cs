...
public class LAppModel : CubismUserModel
{
    ...
    private float _idleTimer = 5.0f;            // 待機モーション間隔タイマ（初期=間隔 → 起動直後に1回再生）
    private const float _idleInterval = 5.0f;   // 待機モーションの最小間隔[秒]
    ...
    public void Update()
    {
        ...
        if (_motionManager.IsFinished())
        {
            if (RandomMotion)
            {
                // 待機モーションの再生間隔を空け、連続再生（ログの飽和）を防ぐ
                _idleTimer += deltaTimeSeconds;
        ...
        if (!motionUpdated)
        {
            // メインモーションの更新がないときのみまばたきを適用（目関連パラメータをモーションが制御中は重複を避ける）
